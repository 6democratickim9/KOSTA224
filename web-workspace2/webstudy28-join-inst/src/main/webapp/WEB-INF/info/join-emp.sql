...
-- constraint(제약조건키워드)  fk_mvc_deptno(제약조건명:유일해야) foreign key(현 테이블 컬럼) references 참조대상테이블or부모테이블(참조할 컬럼)
-- error : parent key not found 참조 무결성 제약 조건 위배 ( 부서 테이블에 존재하지 않는 부서번호로 insert 하려고 했으므로 error ) 
insert into mvc_employee(empno,ename,sal,deptno) values(1,'아이유',400,50);
-- 부서테이블에 존재하는 부서번호로 등록했으므로 정상수행 
...
-- inner join sql : 사원번호 사원명 월급 부서명 지역 정보
