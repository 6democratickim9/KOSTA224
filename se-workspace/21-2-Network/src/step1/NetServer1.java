...
public class NetServer1 {
	public void go() throws IOException {
		...
		//ServerSocket의 accept(): 클라이언트 접속을 대기하다가 접속하면 그 클라이언트와 통신할 일반소켓을 반환
		Socket socket = serverSocket.accept();//클라이언트 접속을 대기
		...
		socket.close();
		serverSocket.close();
		System.out.println("NetServer 클라이언트에게 메세지 출력 후 종료");
	}
